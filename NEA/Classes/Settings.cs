...
namespace NEA
{
    internal class Settings
    {
        ...
        private protected readonly string FilePath = ConfigurationManager.ConnectionStrings["Settings"].ConnectionString;
        ...
        public void LoadSettings()
        {
            if (File.Exists(FilePath))
            {
                try
                {
                    Logon.NotificationTime = Convert.ToInt32(File.ReadLines(FilePath).Skip(1).Take(1).First());    // reads the line where notification time is stored
                    Logon.NotificationStock = Convert.ToInt16(File.ReadLines(FilePath).Skip(3).Take(1).First());   // reads the line where notification time is stored
                    Logon.RecipientEmailAddress = File.ReadLines(FilePath).Skip(5).Take(1).First();                // reads the line where recipient email address is stored
                    Logon.ColourScheme = File.ReadLines(FilePath).Skip(7).Take(1).First();                         // reads the line where colour scheme is stored
                }
        ...
        public void ChangeFileNotifTime(int NewNotificationTimeSeconds)
        {
            try
            {
                if (NewNotificationTimeSeconds > 19)
                {
                    ...
                    string[] notificationLines = File.ReadAllLines(FilePath);
                    notificationLines[1] = Convert.ToString(newNotificationTimeMilliseconds);
                    File.WriteAllLines(FilePath, notificationLines);                                               // writes the new notification time to the settings file
        ...
        public void ChangeFileNotifStock(int NewNotificationStockLevel)
        {
            try
            {
                ...
                File.WriteAllLines(FilePath, notificationLines);                                                   // writes the new notification stock to the settings file
                MessageBox.Show("Notification time changed! Click restart to apply changes.", "Success");
            }
            catch (Exception ex)
            {
        ...
        public void ChangeFileEmail(string NewRecipientEmailAddress)
        {
            try
            {
                ...
                File.WriteAllLines(FilePath, emailLines);                                                      // writes the new email address to the settings file
        ...
        public void ChangeFileColour(string NewColourScheme)
        {
            try
            {
                string[] colourLines = File.ReadAllLines(FilePath);
                colourLines[7] = NewColourScheme;
                File.WriteAllLines(FilePath, colourLines);                                                         // writes the new colour scheme to the settings file
