...
namespace NEA
{
    public partial class FormPurchase : Form
    {
        ...
        {
            InitializeComponent();
            DeliveryDate.MinDate = DateTime.Today;
            ...
            {
                var cmd = new SqlCommand(@"SELECT ProductName FROM Product 
        ...
        private void OrderProductList_SelectedIndexChanged(object Sender, EventArgs E)  // this function has no point going into a class
        {
            ...
            string priceCommand = "SELECT BuyingPrice FROM Product WHERE ProductName = '"
                            + selectedProduct + "';";                                   // SQL query to get the price of the selected database item
            var cmd = new SqlCommand(priceCommand, databaseConnection);
            cmd.ExecuteNonQuery();
            var reader = cmd.ExecuteReader();
            while (reader.Read())
            {
